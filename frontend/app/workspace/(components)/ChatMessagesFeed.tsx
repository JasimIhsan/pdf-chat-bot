...
export function ChatMessagesFeed({ messages, fileState, onStarterPromptClick, onUploadClick }: ChatMessagesFeedProps) {
   ...
   return (
      <MessageScroller className="h-full px-4 py-6 md:px-6 max-w-3xl mx-auto w-full flex flex-col" smooth>
         <div className="space-y-6 pb-20">
            {messages.map((msg) => (
               <Message key={msg.id} from={msg.from} animateIn>
                  <MessageAvatar placeholder={msg.from === "user"}>
                     {msg.from === "assistant" && (
                  ...
                  <MessageContent>
                     ...
                     {msg.thinking ? (
                        ...
                        <MessageBubble variant={msg.from === "user" ? "solid" : "soft"}>
                           <MessageBubbleContent className={msg.from === "user" ? "bg-primary text-primary-foreground font-medium" : "prose dark:prose-invert max-w-none text-sm leading-relaxed"}>
                              {msg.from === "user" ? (
                                 ...
                                 <StreamingResponse status={msg.isStreaming ? "streaming" : "complete"}>
                                    <ReactMarkdown
                                       ...
                                       components={{
                                          ...
                                          blockquote: ({ children }) => <blockquote className="border-l-2 border-primary/60 bg-muted/30 pl-3 py-1 my-2 italic rounded-r-md text-xs">{children}</blockquote>,
                                          code: ({ className, children, ...props }: any) => {
                                             const isInline = !className && !String(children).includes("\n");
                                             return isInline ? (
                                                <code className="px-1.5 py-0.5 rounded-md bg-muted/80 font-mono text-[11px] text-primary border border-border/60" {...props}>
                                                   {children}
                                                </code>
                                             ) : (
                                                <code className="font-mono text-xs text-foreground block whitespace-pre" {...props}>
                                                   {children}
                                                </code>
                                             );
                                          },
                                          pre: ({ children }) => <pre className="p-3 my-2.5 rounded-xl bg-muted/60 dark:bg-zinc-900 border border-border/80 overflow-x-auto text-xs leading-normal">{children}</pre>,
                                       }}
                                    >
                                       {msg.content}
                                    </ReactMarkdown>
