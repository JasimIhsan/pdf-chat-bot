    ...
    <footer className="border-t border-border bg-background">
      <div className="container mx-auto px-4 md:px-8 py-12 max-w-screen-2xl">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="col-span-1 md:col-span-2 space-y-4">
            <Link href="/" className="flex items-center gap-2">
              <Image src="/logo.jpg" alt="PDFChat Logo" width={24} height={24} className="rounded-md" />
              <span className="font-bold">PDFChat</span>
            </Link>
            <p className="text-sm text-muted-foreground max-w-xs">
              Interact with your documents intelligently using state-of-the-art RAG technology.
            </p>
        ...
        <div className="border-t border-border mt-12 pt-8 flex flex-col md:flex-row items-center justify-between text-xs text-muted-foreground">
          <p>© {new Date().getFullYear()} PDFChat. All rights reserved.</p>
          <p className="mt-2 md:mt-0">Designed with Next.js, Tailwind & Framer Motion.</p>
        </div>
      </div>
    </footer>
  );
