...
export function AnimatedToastStack({ toasts, onDismiss, position = "bottom-right", placement, fixed = false, portal, portalRoot, maxVisible = 4, className, classNames, icons, renderToast }: AnimatedToastStackProps) {
   ...
   const stack = (
      <ol
         aria-live="polite"
         aria-atomic="false"
         className={cn("pointer-events-none flex w-[calc(100vw-2rem)] max-w-sm gap-2", isBottom ? "flex-col-reverse" : "flex-col", resolvedPlacement === "fixed" && "fixed z-90", resolvedPlacement === "absolute" && "absolute z-20", resolvedPlacement !== "static" && POSITION_CLASS[position], classNames?.root, className)}
      >
         <AnimatePresence initial={false} mode="popLayout">
            {visibleToasts.map((toast, index) => (
               <ToastItem key={toast.id} toast={toast} index={index} onDismiss={onDismiss} classNames={classNames} icons={icons} renderToast={renderToast} />
            ))}
         </AnimatePresence>
      </ol>
   );
...
const ToastItem = memo(function ToastItem({ toast, index, onDismiss, classNames, icons, renderToast }: { toast: AnimatedToast; index: number; onDismiss?: (id: string) => void; classNames?: ToastClassNames; icons?: Partial<Record<ToastStatus, ReactNode>>; renderToast?: (toast: AnimatedToast) => ReactNode }) {
         ...
         <div className={cn("relative overflow-hidden rounded-2xl border border-border bg-card/95 p-3 shadow-2xl backdrop-blur-xl", classNames?.surface)}>
            {renderToast ? (
               ...
               <div className={cn("flex gap-3", hasDetails ? "items-start" : "items-center")}>
                  <motion.span layout className={cn("inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full", hasDetails && "mt-0.5", STATUS_CLASS[status], classNames?.iconWrap)}>
                  ...
                  <div className={cn("min-w-0 flex-1", classNames?.content)}>
                     <AnimatePresence mode="popLayout" initial={false}>
                        <motion.div
                           ...
                           transition={CONTENT_TRANSITION}
                        >
                           <p className={cn("truncate text-sm font-medium leading-5 text-foreground", classNames?.title)}>{toast.title}</p>
                           {toast.description ? <p className={cn("mt-0.5 line-clamp-2 text-xs leading-4 text-muted-foreground", classNames?.description)}>{toast.description}</p> : null}
                        </motion.div>
                     </AnimatePresence>

                     {toast.action ? (
                        <button type="button" onClick={() => toast.action?.onClick(toast)} className={cn("mt-2 inline-flex h-7 items-center rounded-full bg-primary/6 px-3 text-xs font-medium text-foreground transition-colors hover:bg-primary/10", classNames?.action)}>
                           {toast.action.label}
                        </button>
                     ) : null}
                  </div>
                  ...
                  {canDismiss ? (
                     <button type="button" onClick={() => onDismiss?.(toast.id)} aria-label="Dismiss toast" className={cn("inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-primary/6 hover:text-foreground", classNames?.close)}>
