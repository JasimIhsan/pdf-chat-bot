...
function StepRow({ item }: { item: AgentActivityStep }) {
   ...
   return (
      <div className="flex min-h-5 items-center gap-2 rounded-md px-1 py-0.5 text-xs">
         <span aria-hidden="true" className="grid size-3.5 shrink-0 place-items-center text-muted-foreground/70">
            {state === "complete" ? (
               <Check className="size-3 text-muted-foreground/70" strokeWidth={2} />
            ) : state === "active" ? (
               <span className="relative grid size-2.5 place-items-center">
                  <motion.span className="absolute inset-0 rounded-full bg-primary/20" animate={{ opacity: [0.35, 0.8, 0.35] }} transition={{ duration: 1.5, repeat: Number.POSITIVE_INFINITY }} />
         ...
         <span className={cn("min-w-0 flex-1 text-xs leading-none", state === "pending" ? "text-muted-foreground/50" : state === "active" ? "text-foreground font-medium" : "text-muted-foreground/80")}>{item.label}</span>
         {item.meta ? <span className="shrink-0 text-[11px] text-muted-foreground/50">{item.meta}</span> : null}
      </div>
   );
}
...
function TextRow({ item }: { item: AgentActivityText }) {
  return (
    <div className="rounded-md px-1 py-0.5 text-xs text-muted-foreground">
      {item.content}
    </div>
  );
}
...
function SearchResultRow({
  ...
  const className = cn(
    "flex min-h-6 items-center gap-1.5 rounded-md px-1 py-0.5 text-left text-xs outline-none transition-colors",
      ...
      <AnimatePresence initial>
        {item.moreCount ? (
          <motion.div
            ...
            exit={exit}
            transition={transition}
            className="px-1 py-0.5 pl-6 text-[11px] text-muted-foreground/55"
          >
            +{item.moreCount} more
          </motion.div>
        ) : null}
...
function ToolRow({ item }: { item: AgentActivityTool }) {
  ...
  return (
    <div className="flex min-h-6 min-w-0 items-center gap-2 rounded-md px-1 py-0.5 text-xs leading-none">
      ...
      <span className="min-w-0 flex-1 truncate rounded bg-muted/80 px-2 py-0.5 font-mono text-[11px] text-muted-foreground/70">
        {item.target}
      </span>
...
function TraceRow({ item }: { item: AgentActivityTrace }) {
  return (
    <div className="grid min-h-6 grid-cols-[0.875rem_auto_minmax(0,1fr)] items-center gap-2 rounded-md px-1 py-0.5 text-xs">
      <span
        aria-hidden="true"
        className="grid size-3.5 place-items-center text-muted-foreground/70"
      >
        {item.icon ?? <TraceIcon kind={item.kind} />}
      </span>
      <span className="font-medium text-foreground/90">{item.label}</span>
      {item.detail ? (
        <span className="min-w-0 truncate rounded bg-muted/80 px-2 py-0.5 font-mono text-[11px] text-muted-foreground/70">
          {item.detail}
        </span>
      ) : (
        <span />
