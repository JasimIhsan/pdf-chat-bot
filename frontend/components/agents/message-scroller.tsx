...
export function MessageScroller({
   ...
   const viewport = (
      <section
         ...
         className={cn(
            "h-full overflow-y-auto overscroll-contain outline-none [overflow-anchor:none] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring scroll-smooth",
            navigation === "rail" ? "[-ms-overflow-style:none] scrollbar-none [&::-webkit-scrollbar]:hidden" : "scrollbar-thin",
            viewportClassName,
            navigation === "rail" && railOverflowing && "pr-10"
         )}
      >
         <div ref={contentRef} role="log" aria-live="polite" aria-relevant="additions text" aria-busy={busy} className={contentClassName} {...contentProps}>
            {children}
         </div>
      </section>
   );
